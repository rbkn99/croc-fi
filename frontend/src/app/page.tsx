...
function Diamond({ size, x, y, opacity, rotate = 0 }: { size: number; x: string; y: string; opacity: number; rotate?: number }) {
  return (
    <div className="absolute pointer-events-none" style={{ left: x, top: y, width: size, height: size, transform: `rotate(${rotate}deg)`, opacity }}>
...
export default function Home() {
  ...
  const apys = (products ?? []).map((p) => p.apy ?? 0).filter((a) => a > 0);
  const avgApy = apys.length > 0 ? apys.reduce((s, a) => s + a, 0) / apys.length : null;

  const statsRow = [
    { value: totalProducts > 0 ? String(totalProducts) : "—", label: "Total Assets Minted" },
    { value: avgApy !== null ? `${avgApy.toFixed(2)}%` : "—", label: "Average Yield APY" },
    { value: totalProducts > 0 ? String(totalProducts) : "—", label: "Registered Products" },
  ];

  return (
    <div className="relative overflow-hidden min-h-screen" style={{ background: "var(--color-dark)" }}>
      {/* Radial glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 40%, rgba(23,124,65,0.18) 0%, transparent 70%)" }}
      />
